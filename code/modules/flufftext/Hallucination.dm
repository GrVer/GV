...
/obj/effect/fake_Gagarin
	name = "Yuriy Gagarin"
	icon = null
	icon_state = null //He's not real
	desc = "That's is Gagarin? WHAT?! He died many centuries ago. Oh... that's ghost..."
	//var/mob/myTarget = null
	/*
	An algorithm next:
	1. It spawn in random point in map, which player don't see.
	2. It fly near to player, and say - Over here
	3. It try to get "corners of sight"
	4.1. Not found? Simply fly to wall, and vanish when not in view
	4.2. It fly to it
	5. It moves to black
	6. And vanish
	*/
	...
	New(pos, Target)
		var/turf/aloc = view(world.view, Target)
		..(aloc)
		Target << "\red The Gagarin watch for you!"
		usr << image('grimoriumMobs.dmi', src, "Gagarin_ghost")
		spawn(rand(10,30))
			del(src)
/*
	proc/ToPlayer()
		do
			walk_towards(src, myTarget, 0, 32)
		while (get_dist(myTarget, src) > 1)
		myTarget << sound(pick('sound/hallucinations/over_here1.ogg', 'sound/hallucinations/over_here2.ogg', 'sound/hallucinations/over_here3.ogg'))
		Corner()

	proc/Corner()
		var/turf/seenTurfs = view(world.view, myTarget)
		var/turf/realTurfs = range(world.view, myTarget)
		var/turf/cornerTurfs
		for (var/turf/G in realTurfs)
			for(var/turf/H in range(1, G))
				if (H in seenTurfs)
					cornerTurfs += H
		if (cornerTurfs)
			walk_towards(src, pick(cornerTurfs), 0, 8) //Not fast
		else
			walk_away(src, myTarget, 10, 0, 16) //Some fast
			if (!(src in view(world.view, myTarget)))
				myTarget -= gagarin
				del(src)
				return
		var/turf/seenTurfs2 = view(world.view, src)
		seenTurfs2 = seenTurfs2 - view(world.view, src)
		walk_towards(src, pick(seenTurfs2), 0, 8)
		if (!(src in view(world.view, myTarget)))
			myTarget -= gagarin
			del(src)
*/
